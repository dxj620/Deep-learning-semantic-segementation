...
def RandomCut(image_path, label_path, cutsize_h, cutsize_w, stride, filename):
    ...
    index = 0
    for i in range(0, h_pad_stride // cutsize_h - 2):
        for j in range(0, w_pad_stride // cutsize_w - 2):
            index = index + 1
            topleft_y = i * stride
            topleft_x = j * stride
            ...
            image_save_path = os.path.join(output_rgb_path, filename.replace('.tif', '_%03d.jpg' % index))
...
if __name__ == "__main__":
    ...
    patch_size = 512
    stride = 512
    input_path = r"D:\dataset\Graduation_project\GID\image_val"
    # output_path = r"D:\dataset\Graduation_project\Potsdam\train_512\{}_final".format(patch_size)
    output_path = r"D:\dataset\Graduation_project\GID\test"
    ...
    for filename in tqdm(os.listdir(os.path.join(input_path, input_dir_list[0]))):
        image_path = os.path.join(input_path, input_dir_list[0], filename)
        # label_path = os.path.join(input_path, input_dir_list[1], filename.replace('.jpg', '_label.png'))
        label_path = os.path.join(input_path, input_dir_list[1], filename.replace('.tif', '_label.tif'))
